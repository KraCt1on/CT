...
#isNodule_bool:标记候选是否为真实结节（True=真结节，False=假阳性/非结节）    bool
#diameter_mm:结节的直径（单位：毫米），用于量化大小                        float
#series_uid:关联的CT扫描序列的唯一标识符（DICOM标准），对应.mhd和.raw文件名  str
#center_xyz:结节中心点的三维坐标，格式为 (x, y, z)（对应CT扫描的体素坐标）   tuple
...
class CT:
    def __init__(self, series_uid):
        ...
        self.hu_a = ct_a#保存预处理后的 CT 体素值数组
        #提取空间元数据
    ...
    def getRawCandidate(self, center_xyz, width_irc):
        ...
        center_irc = myUtil.patientCoord2voxelCoord(
            center_xyz,
            self.origin_xyz,#患者坐标系原点
            self.vxSize_xyz,#体素物理尺寸
            self.direction_a,#方向矩阵（3x3）描述体素坐标系到患者坐标系的旋转关系
        )
        slice_list = []
        #存储每个轴（I, R, C）上的切片范围（起始索引和结束索引）
        # 从CT扫描的体素数组中选出候选结节区域
        for axis, center_val in enumerate(center_irc):#遍历三个轴向
            #计算切片范围
            start_ndx = int(round(center_val - width_irc[axis]/2))
            end_ndx = int(start_ndx + width_irc[axis])
            #确保中心点索引在CT数组范围内
            assert center_val >= 0 and center_val < self.hu_a.shape[axis],repr([self.series_uid, center_xyz, self.origin_xyz, self.vxSize_xyz, center_irc, axis])
            #当截取区域左端超出CT数据左边界处理方式：强制从索引0开始截取，保持截取宽度不变（可能丢失部分左侧数据）
            if start_ndx < 0:
                start_ndx = 0
                end_ndx = int(width_irc[axis])
            #当截取区域右端超出CT数据右边界处理方式：强制截取到最大索引，保持截取宽度不变（可能丢失部分右侧数据）
