...
class NoduleAnalysisApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--segmentation-path',
            help="指定语义分割模型的路径",
            nargs='?',
            default='data-unversioned/seg/models/seg/seg_2025-07-03_14.04.11_none.best.state',
        )
        ...
        parser.add_argument('--classification-path',
            help="指定保存分类器模型文件的地址",
            nargs='?',
            default='data-unversioned/nodule/models/nodule-model/cls_2025-07-04_09.32.15_nodule-comment.best.state',
        )
        ...
        parser.add_argument('--malignancy-path',
            ...
            nargs='?',
            default='data-unversioned/tumor/models/tumor_cls/tumor_2025-07-01_10.33.13_finetune-depth2.best.state',
        )
        ...
        self.benign_nodules = 0
        self.malignant_nodules = 0
    ...
    def main(self, series_uid):
        """
        分析指定 series_uid 的 ct(mhd/raw)，输出检测与统计信息。
        """
        # 1. 加载 CT
        ct = getCt(series_uid)
        # 2. 分割
        ...
        # 5. 输出结果
        print(f"在 {series_uid} 中找到的候选结节详情:")
        ...
        # --- 新增代码：在控制台打印最终统计结果 ---
        print("\n" + "=" * 30)
        print(f"分析结果统计 ({series_uid}):")
        print(f"  检测到的总结节数: {nodule_count}")
        if self.malignancy_model:
            print(f"    - 恶性结节数: {malignant_count}")
            print(f"    - 良性结节数: {benign_count}")
        else:
            print("  (未加载恶性/良性分类模型)")
        print("=" * 30 + "\n")

        self.total_nodules = nodule_count
        self.benign_nodules = benign_count
        self.malignant_nodules = malignant_count
    ...
    def get_total_nodules(self):
        """获取总结节数"""
        return self.total_nodules

    def get_malignant_nodules(self):
        """获取恶性结节数"""
        return self.malignant_nodules

    def get_benign_nodules(self):
        """获取良性结节数"""
        return self.benign_nodules
...
if __name__ == "__main__":
    # 假设 app 为你的 NoduleAnalysisApp 实例
    series_uid = "1.3.6.1.4.1.14519.5.2.1.6279.6001.100621383016233746780170740405"  # 换成你要分析的UID
